...
class LatentUpsamplerConfigurator(ModelConfigurator[LatentUpsampler]):
    ...
    def from_config(cls: type[LatentUpsampler], config: dict) -> LatentUpsampler:
        ...
        spatial_scale = config.get("spatial_scale", config.get("rational_spatial_scale", 2.0))
        rational_resampler = config.get("rational_resampler", "rational_spatial_scale" in config)
