...
_LTX_V2_ROOT = _REPO_ROOT / "ltx_v2"
...
def gather_object_list_to_all(items: list[dict], world_size: int) -> list[dict]:
    if world_size == 1:
        return list(items)
    outputs: list[list[dict]] = [None for _ in range(world_size)]
    dist.all_gather_object(outputs, items)
    merged: list[dict] = []
    for x in outputs:
        merged.extend(x)
...
def is_qwen3omni_reward_config(reward_config) -> bool:
    keys = set(reward_config.keys()) if hasattr(reward_config, "keys") else set()
    return bool(keys & {"qwen3omni_audio_quality", "qwen3omni_video_quality", "qwen3omni_shared"})
...
class TextPromptDataset(Dataset):
    def __init__(self, dataset, split="train", extra_jsonl_files=None, extra_jsonl_repeat=1):
        self.metadatas = []
        self.prompts_v = []
        self.prompts_a = []
        self.prompts_va = []

        file_path = os.path.join(dataset, f"{split}_metadata.jsonl")
        if not os.path.exists(file_path):
            file_path = dataset
        files_to_load = []
        if os.path.exists(file_path):
            files_to_load.append((file_path, 1))
        repeat = max(1, int(extra_jsonl_repeat))
        for extra_path in extra_jsonl_files or []:
            extra_path = os.path.expanduser(str(extra_path))
            if os.path.exists(extra_path):
                files_to_load.append((extra_path, repeat))
            elif logger.isEnabledFor(logging.WARNING):
                logger.warning("Extra train jsonl not found, skipping: %s", extra_path)

        if not files_to_load:
            raise FileNotFoundError(f"No training metadata file found for split={split}: {dataset}")

        for path, file_repeat in files_to_load:
            if Path(path).suffix.lower() == ".csv":
                self._load_csv(path, repeat=file_repeat)
            else:
                self._load_jsonl(path, repeat=file_repeat)

        self.file_path = files_to_load[0][0]

    def _append_item(self, item):
        fallback_prompt = item.get("prompt_av") or item.get("prompt", "")
        self.metadatas.append(item)
        self.prompts_v.append(item.get("prompt_v") or fallback_prompt)
        self.prompts_a.append(item.get("prompt_a", ""))
        self.prompts_va.append(fallback_prompt)

    def _load_jsonl(self, path, repeat=1):
    ...
    def _load_csv(self, path, repeat=1):
        with open(path, "r", encoding="utf-8", newline="") as f:
            reader = csv.DictReader(f)
            required_columns = {"video_text", "audio_text", "text"}
            missing_columns = required_columns.difference(reader.fieldnames or [])
            if missing_columns:
                raise ValueError(f"CSV metadata is missing columns {sorted(missing_columns)}: {path}")
            for row in reader:
                item = dict(row)
                item["prompt_v"] = (row.get("video_text") or row.get("text") or "").strip()
                item["prompt_a"] = (row.get("audio_text") or "").strip()
                item["prompt_av"] = (row.get("text") or "").strip()
                if not item["prompt_av"]:
                    continue
                for _ in range(repeat):
                    self._append_item(item)
...
class DistributedLocalKRepeatSampler(Sampler):
    """Keep every repeated prompt group on one rank for pairwise rewards."""
    ...
    def __init__(self, dataset, batch_size, k, num_replicas, rank, seed=0):
        ...
        if self.batch_size % self.k != 0:
            raise ValueError(f"local grouped sampler requires batch_size % k == 0, got {batch_size=} {k=}")
    ...
    def __iter__(self):
        while True:
            g = torch.Generator()
            g.manual_seed(self.seed + self.epoch)
            total_groups = self.num_replicas * self.groups_per_rank
            perm = torch.randperm(len(self.dataset), generator=g).tolist()
            if len(perm) < total_groups:
                repeats = (total_groups + len(perm) - 1) // len(perm)
                perm = (perm * repeats)[:total_groups]
            else:
                perm = perm[:total_groups]
            st = self.rank * self.groups_per_rank
            ed = st + self.groups_per_rank
            batch = []
            for idx in perm[st:ed]:
...
class DistributedRankGroupedKRepeatSampler(Sampler):
    """Share each prompt across a fixed rank group and split its samples."""

    def __init__(self, dataset, batch_size, k, num_replicas, rank, rank_group_size, seed=0):
        self.dataset = dataset
        self.batch_size = batch_size
        self.k = k
        self.num_replicas = num_replicas
        self.rank = rank
        self.rank_group_size = rank_group_size
        self.seed = seed
        if self.rank_group_size <= 0 or self.num_replicas % self.rank_group_size != 0:
            raise ValueError(
                f"rank_group_size must divide num_replicas, got {rank_group_size=} {num_replicas=}"
            )
        if self.k <= 0 or self.k % self.rank_group_size != 0:
            raise ValueError(f"k must be divisible by rank_group_size, got {k=} {rank_group_size=}")
        self.samples_per_prompt_per_rank = self.k // self.rank_group_size
        if self.batch_size % self.samples_per_prompt_per_rank != 0:
            raise ValueError(
                "rank-grouped sampler requires batch_size divisible by samples_per_prompt_per_rank, "
                f"got {batch_size=} {self.samples_per_prompt_per_rank=}"
            )
        self.prompt_groups_per_rank = self.batch_size // self.samples_per_prompt_per_rank
        self.num_rank_groups = self.num_replicas // self.rank_group_size
        self.rank_group_index = self.rank // self.rank_group_size
        self.epoch = 0
    ...
    def __iter__(self):
        while True:
            ...
            total_prompt_groups = self.num_rank_groups * self.prompt_groups_per_rank
            permutation = torch.randperm(len(self.dataset), generator=generator).tolist()
            if len(permutation) < total_prompt_groups:
                repeats = (total_prompt_groups + len(permutation) - 1) // len(permutation)
                permutation = (permutation * repeats)[:total_prompt_groups]
            else:
                permutation = permutation[:total_prompt_groups]
            start = self.rank_group_index * self.prompt_groups_per_rank
            selected = permutation[start : start + self.prompt_groups_per_rank]
            batch = []
            for index in selected:
                batch.extend([index] * self.samples_per_prompt_per_rank)
            yield batch
...
class DistributedRankGroupedEvalSampler(Sampler):
    """Give every rank in a reward group the same finite eval prompts."""

    def __init__(self, dataset, num_replicas, rank, rank_group_size):
        self.dataset = dataset
        self.num_replicas = num_replicas
        self.rank = rank
        self.rank_group_size = rank_group_size
        if self.rank_group_size <= 0 or self.num_replicas % self.rank_group_size != 0:
            raise ValueError(
                f"rank_group_size must divide num_replicas, got "
                f"{rank_group_size=} {num_replicas=}"
            )
        self.num_rank_groups = self.num_replicas // self.rank_group_size
        self.rank_group_index = self.rank // self.rank_group_size
        self.indices = list(range(self.rank_group_index, len(self.dataset), self.num_rank_groups))
...
def _update_param_pairs(source_params, target_params, decay: float | None = None):
    """Copy or EMA-update matching local parameter shards without full unsharding."""
    if len(source_params) != len(target_params):
        raise RuntimeError(f"parameter pair count mismatch: {len(source_params)} vs {len(target_params)}")
    with torch.no_grad():
        for index, (source, target) in enumerate(zip(source_params, target_params, strict=True)):
            if source.shape != target.shape:
                raise RuntimeError(
                    f"parameter shard shape mismatch at index {index}: {tuple(source.shape)} vs {tuple(target.shape)}"
                )
            if decay is None:
                target.copy_(source)
            else:
...
def save_ckpt(
    ...
    if use_fsdp:
        ...
        if use_lora:
            # FSDP flat handles contain both the trainable "default" adapter and
            # frozen "old" adapter, while Adam only owns "default". PyTorch's
            # full optim_state_dict conversion rejects that subset. Persist each
            # rank's local optimizer shard instead; resume requires the same
            # FSDP world size and rank layout.
            torch.save(optimizer.state_dict(), ckpt_dir / f"optimizer_rank_{rank:05d}.pt")
            optim_sd = None
            dist.barrier()
        else:
        ...
        if rank == 0:
            state = {
                "optimizer": optim_sd,
                "optimizer_format": "per_rank" if use_lora else "fsdp_full",
...
def main(_):
    ...
    writer = None
    if os.path.isabs(config.run_name) or os.sep in config.run_name:
        log_dir = config.run_name
    else:
    ...
    resume_rank_optimizer_path = (
        resume_root / f"optimizer_rank_{rank:05d}.pt" if resume_root is not None else None
    )
    ...
    resume_optimizer_is_local = False
    ...
    if resume_rank_optimizer_path is not None and resume_rank_optimizer_path.exists():
        resume_optimizer_state = torch.load(resume_rank_optimizer_path, map_location="cpu")
        resume_optimizer_is_local = True
        logger.info("Restoring per-rank optimizer state from %s", resume_rank_optimizer_path)
    ...
    # Data
    # =========================================================================
    extra_train_jsonls = _cfg_get(config, "extra_train_jsonls", None) or _cfg_get(config, "demo_prompt_jsonls", None) or []
    extra_train_repeat = int(_cfg_get(config, "extra_train_repeat", 1))
    train_dataset = TextPromptDataset(
        _cfg_get(config, "train_dataset", ""),
        "train",
        extra_jsonl_files=extra_train_jsonls,
        extra_jsonl_repeat=extra_train_repeat,
    )
    ...
    prompt_rank_group_size = int(_cfg_get(config, "sample.prompt_rank_group_size", 1))
    if prompt_rank_group_size > 1:
        train_sampler = DistributedRankGroupedKRepeatSampler(
            **sampler_kwargs,
            rank_group_size=prompt_rank_group_size,
        )
    elif bool(_cfg_get(config, "sample.local_prompt_grouping", False)):
        train_sampler = DistributedLocalKRepeatSampler(**sampler_kwargs)
    else:
        train_sampler = DistributedKRepeatSampler(**sampler_kwargs)
    train_loader = DataLoader(train_dataset, batch_sampler=train_sampler, collate_fn=train_dataset.collate_fn, num_workers=0)
    if is_main_process(rank) and extra_train_jsonls:
        logger.info(
            "Merged %d extra train jsonl files (%dx repeat each) into dataset (total=%d prompts)",
            len(extra_train_jsonls),
            extra_train_repeat,
            len(train_dataset),
        )

    test_batch_size = int(_cfg_get(config, "sample.test_batch_size", 1))
    if prompt_rank_group_size > 1:
        if test_batch_size != 1:
            logger.info(
                "Overriding sample.test_batch_size=%d with 1 for Qwen3-Omni "
                "rank-grouped pairwise evaluation.",
                test_batch_size,
            )
            test_batch_size = 1
        test_sampler = DistributedRankGroupedEvalSampler(
            test_dataset,
            num_replicas=world_size,
            rank=rank,
            rank_group_size=prompt_rank_group_size,
    ...
    use_qwen3omni_reward = is_qwen3omni_reward_config(reward_config)
    reward_fn = flow_grpo.rewards.multi_score(device, reward_config)
    ...
    if use_lora and not old_adapter_restored:
        if use_fsdp:
            _update_param_pairs(trainable_params, old_trainable_params)
        else:
            _update_param_pairs(trainable_params, old_trainable_params)
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for bi in tqdm(range(num_batches_per_epoch), desc=f"Epoch {epoch}: sampling", disable=not is_main_process(rank)):
            ...
            if use_qwen3omni_reward:
                for i, item in enumerate(local_samples):
                    item["sample_key"] = f"{rank}:{len(samples) + i}"
        ...
        if use_qwen3omni_reward and len(samples) > 0:
            local_records = []
            for item in samples:
                temp_video_path = item.get("temp_video_path")
                if temp_video_path is None:
                    raise ValueError("Qwen3-Omni reward requires temp_video_path for every sample")
            ...
            all_records = gather_object_list_to_all(local_records, world_size)
            if is_main_process(rank):
                qwen_reward_log_dir = os.path.join(log_dir, "qwen3omni_reward_logs")
                qwen_reward_by_key = flow_grpo.rewards.qwen3omni_pairwise_score_records(
                    all_records,
                    log_dir=qwen_reward_log_dir,
                    epoch=epoch,
                    global_step=global_step,
                )
            else:
                qwen_reward_by_key = None
            if world_size > 1:
                wrapped_rewards = [qwen_reward_by_key]
                dist.broadcast_object_list(wrapped_rewards, src=0)
                qwen_reward_by_key = wrapped_rewards[0]
            assert qwen_reward_by_key is not None

            for i, item in enumerate(samples):
                details = qwen_reward_by_key.get(item["sample_key"], {})
                item["rewards"] = {
                    "qwen3omni_audio_quality": float(details.get("qwen3omni_audio_quality", 0.0)),
                    "qwen3omni_video_quality": float(details.get("qwen3omni_video_quality", 0.0)),
                    "qwen3omni_shared": float(details.get("qwen3omni_shared", 0.0)),
                }
                item["reward"] = float(details.get("avg", sum(item["rewards"].values())))

                temp_video_path = item.pop("temp_video_path", None)
                temp_audio_path = item.pop("temp_audio_path", None)
                if temp_video_path is not None and os.path.exists(temp_video_path):
                    if should_save and sample_save_dir is not None:
                        rew_val = float(item["reward"])
                        safe_prompt = _safe_prompt_name(item["prompt"], max_len=15)
                        ext = os.path.splitext(temp_video_path)[1]
                        filename = f"rank{rank}_s{i}_rew_{rew_val:.3f}_{safe_prompt}{ext}"
                        final_save_path = os.path.join(sample_save_dir, filename)
                        try:
                            shutil.move(temp_video_path, final_save_path)
                        except Exception as e:
        ...
        # torch.cat/stack above created independent packed tensors. Release the
        # per-sample rollout tensors instead of keeping a duplicate GPU copy.
        del samples
        cpu_offload_train_samples = bool(_cfg_get(config, "train.cpu_offload_samples", False))
        if cpu_offload_train_samples:
            packed = {key: value.detach().cpu() for key, value in packed.items()}
        ...
        for inner in range(num_inner):
            perm = torch.randperm(local_n, device="cpu" if cpu_offload_train_samples else device)
            micro_batches = [perm[i: i + train_bs] for i in range(0, local_n, train_bs)]
            info = defaultdict(list)

            for mb_idx in micro_batches:
                if cpu_offload_train_samples:
                    b = {key: value[mb_idx].to(device, non_blocking=True) for key, value in packed.items()}
                else:
                    b = {key: value[mb_idx] for key, value in packed.items()}
                ...
                for t_idx in tqdm(torch.randperm(num_train_t, device=device)):
                    ...
                    if use_fsdp:
                        # FSDP no_sync() retains full, unsharded gradients until the
                        # next synchronized backward. Always reduce-scatter here so
                        # timestep accumulation remains sharded on memory-limited GPUs.
        ...
        if use_lora:
            if use_fsdp:
                ...
                _update_param_pairs(trainable_params, old_trainable_params, decay=decay)
            else:
                decay = return_decay(global_step, int(_cfg_get(config, "decay_type", 1)))
                _update_param_pairs(trainable_params, old_trainable_params, decay=decay)
        else:
        ...
        if (epoch % eval_freq == 0) and not debug:
            ...
            for bidx, (prompts, metas) in enumerate(test_loader):
                ...
                reward_input = reward_inputs if use_video_reward else torch.stack(reward_inputs).to(device)
                eval_group_size = len(reward_inputs) * int(
                    _cfg_get(config, "sample.prompt_rank_group_size", 1)
                )
                eval_metas = []
                for meta in metas:
                    eval_meta = dict(meta) if isinstance(meta, dict) else {"raw_metadata": meta}
                    eval_meta["_qwen3omni_expected_group_size"] = eval_group_size
                    eval_metas.append(eval_meta)
                rewards, _ = reward_fn(reward_input, prompts, eval_metas, only_strict=False)
