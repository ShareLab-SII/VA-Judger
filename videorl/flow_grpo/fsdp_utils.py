...
def fsdp_wrapper(model, fsdp_config, get_transformer_layer_cls, ignored_modules=None):
    ...
    backward_prefetch = (
        None
        if str(fsdp_config.backward_prefetch).upper() in ("NONE", "OFF", "DISABLED")
        else BackwardPrefetch[str(fsdp_config.backward_prefetch).upper()]
    )
...
#             pt_save_path = os.path.join(save_path, "xvideo_dpo.pt")
#             torch.save(state_dict, pt_save_path)
#             print(f"Model saved as torch .pt: {pt_save_path}")
#         del state_dict
...
def save_fsdp_checkpoint(save_dir, model, global_step, rank):
    ...
    with FSDP.state_dict_type(
        ...
        state_dict = model.state_dict()
        if rank == 0:
            save_file(state_dict, os.path.join(save_path, "xvideo_nft.safetensors"))
            print(f"Model saved as safetensors: {save_path}/xvideo_nft.safetensors")
        del state_dict
